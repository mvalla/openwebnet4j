...
public class BUSConnector extends OpenConnector {
    ...
    public static final String CMD_SESSION_ALT =
            "*99*9##"; // new command session defined in OWN official docs
    ...
    static final int SOCKET_CONNECT_TIMEOUT =
            5000; // (ms) time to wait while connecting a new socket to the
    // gateway
    static final int MON_SOCKET_READ_TIMEOUT =
            120000; // (ms) time to wait while reading from MON socket

    static final int CMD_SOCKET_READ_TIMEOUT =
            30000; // (ms) time to wait while reading from CMD socket

    public static final int MON_KEEPALIVE_TIMER =
            90000; // (ms) timer to send a keepalive on MON channel

    public static final int HANDSHAKE_TIMEOUT =
            2000; // (ms) timeout before handshake must be completed
    ...
    private void openConnection(String type) throws OWNException {
        logger.debug(
                "##BUS-conn## Establishing {} connection to BUS Gateway on {}:{}...",
                type,
                host,
                port);
        try {
        ...
        try {
            ...
            try {
                ...
                throw new IOException(
                        "Cannot create NEW CMD connection to send message " + frame, oe);
            }
            ...
            try {
                ...
                logger.warn(
                        "##BUS-conn## sendCmdAndReadResp() returned exception ({}) using NEW connection",
                        e.getMessage());
                throw (e);
            }
        }
    ...
    private Response sendCmdAndReadResp(String frame, boolean reopen)
            throws IOException, FrameException {
        ...
        while (!res.hasFinalResponse()) {
            ...
            if (fr != null) {
                try {
                    ...
                    msgLogger.debug(
                            "BUS-CMD   <<=X   {} ignoring unsupported response frame ({})",
                            fr,
                            ufe.getMessage());
                }
            } else {
    ...
    private void startMonKeepaliveTimer() {
        ...
        monKeepaliveTimer.schedule(
                new TimerTask() {
                    @Override
                    public void run() {
                        if (monSk.isClosed()) {
                            logger.debug(
                                    "##BUS-conn## socket is closed, cancelling keepalive timer: {}",
                        ...
                        try {
                            ...
                            logger.debug(
                                    "##BUS-conn## could not send MON keepalive ACK: exception={}",
                                    e.getMessage());
                        }
                    }
                },
    ...
    private FrameChannel connectSocket(String type) throws IOException {
        ...
        if (type.equals(MON_TYPE)) {
            monChannel =
                    new FrameChannel(sk.getInputStream(), sk.getOutputStream(), "BUS-" + MON_TYPE);
            monSk = sk;
            return monChannel;
        } else {
            cmdChannel =
                    new FrameChannel(sk.getInputStream(), sk.getOutputStream(), "BUS-" + CMD_TYPE);
            cmdSk = sk;
            return cmdChannel;
        }
    }
                ...
                new TimerTask() {
                    ...
                    public void run() {
                        if (!frCh.handshakeCompleted) {
                            logger.warn(
                                    "(HS) ... handshake not completed but timeout expired, closing {} channel",
                                    frCh.getName());
                            frCh.disconnect();
    ...
    private void doHandshake(FrameChannel frCh, String type) throws IOException, OWNAuthException {
        ...
        if (!(OpenMessage.FRAME_ACK.equals(fr))) {
            ...
            throw new OWNAuthException(
                    "Could not open BUS-"
                            ...
                            + ":"
                            + port
                            + " (no ACK received at STEP-1, received: "
                            + fr
                            + ")");
        }
        ...
        if (OpenMessage.FRAME_ACK.equals(fr)) {
            ...
            hsLogger.debug(
                    "(HS) ... STEP-2: NO_AUTH: second ACK received, GW has no pwd ==HANDSHAKE COMPLETED==");
            ...
            hsLogger.warn(
                    "(HS) ... STEP-2: cannot authenticate with gateway (unexpected answer: {})",
                    fr);
            throw new OWNAuthException(
                    "Cannot authenticate with gateway: handshake failed at STEP-2 (unexpected answer: "
                            + fr
                            + ")");
        }
    }

    private void doOPENHandshake(String nonceFrame, FrameChannel frCh)
            throws IOException, OWNAuthException {
        ...
        try {
            pwdMessage =
                    OpenMessage.FRAME_START_DIM
                            + Auth.calcOpenPass(pwd, nonce)
                            + OpenMessage.FRAME_END;
        } catch (NumberFormatException e) {
            hsLogger.warn(
                    "(HS) ... STEP-3: OPEN_AUTH: invalid gateway password. Password must contain only digits");
            throw new OWNAuthException(
                    "Invalid gateway password. Password must contain only digits (OPEN_AUTH)");
        }
        ...
        if (OpenMessage.FRAME_ACK.equals(fr)) {
            ...
            throw new OWNAuthException(
                    "Password not accepted by gateway, check password configuration (OPEN_AUTH)");
        }
    }

    private void doHMACHandshake(String hmacType, FrameChannel frCh)
            throws IOException, OWNAuthException {
        hsLogger.debug(
                "(HS) ... STEP-2: HMAC_AUTH: HMAC type received: {}, sending ACK ... ", hmacType);
        ...
        if (matcher.find()) {
            ...
            if (OpenMessage.FRAME_NACK.equals(fr)) {
                ...
                throw new OWNAuthException(
                        "Password not accepted by gateway, check password configuration (HMAC)");
            } else {
                ...
                if (matcher.find()) {
                    ...
                    hsLogger.trace(
                            "(HS) ... STEP-4: HMAC_AUTH: final hash HMAC(Ra, Rb, Kab) received: {} ...",
                            hmacRaRbKab);
                    if (Auth.calcSHA256(ra + rb + kab).equals(hmacRaRbKab)) {
                        hsLogger.trace(
                                "(HS) ... STEP-4: HMAC_AUTH:  HMAC(Ra, Rb, Kab) --MATCH--, sending ACK ...");
                        frCh.sendFrame(OpenMessage.FRAME_ACK);
                        hsLogger.info("(HS) {} HS==>>>> {}", frCh.getName(), OpenMessage.FRAME_ACK);
                        hsLogger.debug(
                                "(HS) ... STEP-4: HMAC_AUTH: final ACK sent ==HANDSHAKE COMPLETED==");
                        return;
                    } else {
                        hsLogger.warn(
                    ...
                    hsLogger.warn(
                            ...
                            fr);
                    throw new OWNAuthException(
                            "Handshake failed, invalid HMAC(Ra, Rb, Kab) received from GW at HMAC STEP-4: "
                                    + fr);
                }
            }
        } else {
            hsLogger.warn(
                    "(HS) ... STEP-3: HMAC_AUTH: HANDSHAKE FAILED, invalid Ra received. Response={}",
                    fr);
            throw new OWNAuthException(
                    "Handshake failed, no Ra received from GW at HMAC STEP-3: " + fr);
        }
    }
